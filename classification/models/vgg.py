...
from .sfam import NonLocalBlock
from .gd import FeatureFusion
...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes=1000, init_weights=True, att_dir='./runs/', training_epoch=15):
        ...
        self.extra_convs_2=nn.Conv2d(512, 512, kernel_size=3, padding=1)
        # self.extra_convs_2 = GloRe_Unit_2D(512, 64)

        self.extra_convs_4 = NonLocalBlock(512,512)
        self.extra_convs_5 = FeatureFusion(512,512)
        self.extra_convs_3 = nn.Sequential(
            nn.ReLU(True),
            nn.Conv2d(512, 17, 1)
        )
        # self.extra_convs_5 = nn.Conv2d(in_channels=17, out_channels=512, kernel_size=1, stride=1,
        #                           padding=0, bias=False)
        #
        # self.extra_convs_6 = NonLocalBlock(512,512)
        # self.extra_convs_7 = nn.Conv2d(in_channels=512, out_channels=17, kernel_size=1, stride=1,
        #                                padding=0, bias=False)
    ...
    def forward(self, x, epoch=1, label=None, img_name=None):
        x = self.features(x)
        x1 = self.extra_convs_1(x)
        #print(x1.shape)
        x2 = self.extra_convs_2(x1)
        # print(x2.shape)


        x3 = self.extra_convs_4(x1,x2)
        x4 = self.extra_convs_4(x2,x1)
        #print(x3.shape)
        #x=x3+x4
        x = self.extra_convs_5(x3,x4)
        #print(x.shape)
        x=x.clone()
        # print(x3.shape)
        ...
        #print(x.shape)
        # x=self.extra_convs_5(x)
        # x=self.extra_convs_6(x3,x)
        #
        # x = self.extra_convs_7(x)
        # print(x.shape)
        self.map1 = x.clone()

...
cats = ['background', 'architecture', 'stairs', 'window', 'pillar', 'tsun-shou', 'door', 'stone step', 'architrave', 'plaque', 'dou-gong',
       'ridge', 'pagoda', 'stone lion', 'stone elephant', 'fence', 'censer', 'stone tablet']

data_path = './'
train_lst_path = data_path + 'data/train_set.txt'
sal_path = data_path + 'data/saliency_aug/'
att_path = data_path + 'runs/exp2/attention/'
last_att_path = data_path + 'runs/exp1/attention/'
att_clustering = data_path + 'runs/exp1/clustering/'
...
def gen_gt(index):
    ...
    gt = np.zeros((18, height, width), dtype=np.float32)
    added_gt = np.zeros((18, height, width), dtype=np.float32)
    added_gt[0] = 0.5
    ...
    gt[bg > 0] = 255
    weight_att = 0.75  # 注意力图的权重
    weight_clustered = 0.25  # 谱聚类图的权重
    # POM
    for i in range(len(fields) - 1):
        k = i + 1
        cls = int(fields[k])
        att_name = last_att_path + name + '_' + str(cls) + '.png'
        att_clustering_name = att_clustering + name + '_' + str(cls) + '.png'
        if not exists(att_name):
            continue
        if not exists(att_clustering_name):
            continue

        # normalize attention to [0, 1]
        att = cv2.imread(att_name, 0)
        clustered_labels = cv2.imread(att_clustering_name, 0)
        clustered_labels = clustered_labels / np.max(clustered_labels)
        att = (att - np.min(att)) / (np.max(att) - np.min(att) + 1e-8)
        integrated_att = (att * weight_att) + (clustered_labels * weight_clustered)
        position = [gt == (cls + 1)]

        temp = integrated_att[tuple(position)]
        if np.sum(temp) != 0:
            flt_thr = np.median(temp)
        else:
            position = [integrated_att > 0.3]
            if np.sum(position) != 0:
                temp = integrated_att[tuple(position)]
                temp_median = np.median(temp)
                position = [integrated_att > temp_median]
                temp = integrated_att[tuple(position)]
                flt_thr = np.median(temp)
            else:
                flt_thr = 1

        select_position = np.where(integrated_att > flt_thr, 1, 0)
